...
def step_impl(context, preciomin):
    precio_min = context.driver.find_element(By.ID, "id_precio_min").send_keys(int(preciomin))


@given(u'selecciona un precio maximo "{preciomax}"')
def step_impl(context, preciomax):
    precio_max = context.driver.find_element(By.ID, "id_precio_max").send_keys(int(preciomax))


@given(u'da click en aplicar filtros')
def step_impl(context):
    aplicar = context.driver.find_element(By.XPATH, "/html/body/div/div[2]/div/div[2]/form/table/tbody/tr[2]/td[7]/div/button/i").click()
...
def step_impl(context, Municipio):
    municipio = context.driver.find_element(By.ID, "municipio").send_keys(Municipio)
