...
def step_impl(context, datos_propiedad):  # noqa: F811
    ...
    for propiedad in propiedades:
        if (propiedad.find_element(
            By.CLASS_NAME,
                'card-title').text == datos_propiedad):
...
def step_impl(context, mensaje_descripcion):  # noqa: F811
    context.driver.find_element(
...
def step_impl(context, numero_estrellas):  # noqa: F811
...
def step_impl(context, texto):  # noqa: F811
    if texto in context.driver.find_element(
            By.XPATH,
            '/html/body/div/div[2]/div/form/div[5]').text:
        return True
    else:
        return False
