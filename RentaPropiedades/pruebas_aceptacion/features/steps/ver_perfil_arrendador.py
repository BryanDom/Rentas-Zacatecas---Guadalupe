...
def step_impl(context, ocupacion):
    oc = context.driver.find_element(By.XPATH, '/html/body/div/div[2]/div/div/div/div[2]/div/p[4]').text
    assert ocupacion in oc


@then(u'puede ver sus preferencias de arrrendamiento "{preferencia}"')
def step_impl(context, preferencia):
    pr = context.driver.find_element(By.XPATH, '/html/body/div/div[2]/div/div/div/div[2]/div/p[6]').text
