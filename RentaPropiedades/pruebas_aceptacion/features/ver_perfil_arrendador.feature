Característica: Visualizar perfil de arrendador
    Como arrendador del sistema de rentas
    quiero ver mi perfil
    para consultar mi información personal.

    Escenario: El arrendador ve su perfil exitosamente
    Dado que el arrendador ingresa a la url "http://localhost:8000/"
    Y escribe su correo "luisperez@gmail.com" y su contraseña "1234" 
    Y presiona el botón de Log in
    Y presiona el botón de ver Perfil arrendador
    Entonces puede ver su nombre completo "Luis López Pérez"
    Y puede ver su edad "40"
    Y puede ver su sexo "Masculino"
    Y puede ver su ocupacion "Abogado"
    Y puede ver su teléfono "1234567890"
    Y puede ver sus preferencias de arrrendamiento "Muchachos responsables"