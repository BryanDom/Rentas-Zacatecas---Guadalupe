...
def editar_perfil_estudiante(request):
    ...
    return render(request, 'editar_perfil_estudiante.html', {'form': form, 'user_id': id})
...
def editar_perfil_arrendador(request):
    ...
    if request.method == 'POST':
        ...
        form = FormArrendador(instance=arrendador, initial={'foto_perfil': imagen_perfil_url})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if user.check_password(form.cleaned_data['password']):
                # Si el formulario es válido y la contraseña es correcta, inicia sesión
                response = self.form_valid(form)
                self.request.session['user_id'] = user.id  # Guarda el ID del usuario en la sesión
                return response
            else:
            ...
            try:
                ...
                if not user.has_usable_password():
                    ...
                    user.delete()
                    messages.warning(request, 'Aún no te has registrado correctamente. Inicia tu registro :).')
                    return self.form_invalid(form)
                else:
                    messages.warning(request, 'Contraseña incorrecta :(')
                    return self.form_invalid(form)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not request.FILES.get('foto_perfil'):
                # Si no se proporcionó una imagen, establece la imagen predeterminada
                imagen_predeterminada = 'perfiles/1144760.png'
                arrendador.foto_perfil.name = imagen_predeterminada  # Establece la ruta de la imagen predeterminada
